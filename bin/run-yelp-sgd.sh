...
SEED=1213279863
NUM_PARTITIONS=1024
TEST_PARTITIONS=1024
LOG_SUFFIX=`date +"%Y_%m_%d_%H_%M_%S"`
#1e-3 is too slow
#1e1 is too slow
#1e3 iter 8 is 1.572068 
#2.5e3 diverges
#5e3 diverges
#1e4 diverges
#1e5 diverges
...
OMP_NUM_THREADS=1 KEYSTONE_MEM=180g ./bin/run-pipeline.sh \
  ...
  --seed $SEED 2>&1 | tee /root/logs/yelp-solver-$SOLVER-lambda-$LAMBDA-numHash-$NUM_HASH_FEATURES-numIters-$NUM_ITERS-mini-$MINI_BATCH_FRACTION-step-$STEP_SIZE-dampen-$SGD_DAMPEN-logs-"$LOG_SUFFIX".log
