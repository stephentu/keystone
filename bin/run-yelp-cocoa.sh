...
OMP_NUM_THREADS=1 KEYSTONE_MEM=180g ./bin/run-pipeline.sh \
  ...
  --seed $SEED 2>&1 | tee /root/logs/yelp-solver-$SOLVER-lambda-$LAMBDA-numHash-$NUM_HASH_FEATURES-numIters-$NUM_ITERS-cocoaLocal-"$COCOA_LOCAL_ITERS"-logs-"$LOG_SUFFIX".log
